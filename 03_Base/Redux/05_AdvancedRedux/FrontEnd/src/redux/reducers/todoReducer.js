...
import { deleteNoteAsync } from "./noteReducer";
...
export const getToggleUpdateAsync = createAsyncThunk(
    "todo/getToggleUpdated", async (payload)=> {
        try{
            const response = await fetch("http://localhost:4100/api/todos/toggle",{
                method : 'POST',
                headers: {
...
const todoSlice = createSlice({
    ...
    extraReducers:(builder)=>{
        ...
        .addCase(getToggleUpdateAsync.fulfilled,(state,action)=>{
            console.log("asdfasdfsdfasdfasdf" , action.payload);
